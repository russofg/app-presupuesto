...
import { getLastVoucher, requestCAE } from "@/lib/arca-wsfe";
...
function formatDateArca(date?: string): string {
...
async function generateQrDataUrl(params: {
  ...
  const url = `https://www.afip.gob.ar/fe/qr/?p=${encoded}`;
...
export async function POST(request: Request) {
  try {
    ...
    const {
      clienteNombre,
      clienteDoc,
      clienteDocTipo = 96, // 96=DNI, 80=CUIT
      concepto,
      importe,
      fecha,
      conceptoTipo = 2, // 1: Productos, 2: Servicios, 3: Productos/Servicios
    ...
    const { cert, key, cuit, ptoVenta, production, nombre } = getArcaCredentials();
    ...
    // Step 2: Get next voucher number (tipo 11 = Factura C)
    const lastVoucher = await getLastVoucher(cuit, ptoVenta, 11, token, sign, production);
    const nextVoucher = lastVoucher + 1;

    const fechaFactura = formatDateArca(fecha);
    const importeNum = parseFloat(String(importe));

    // For Concepto 2 or 3, format the service dates, otherwise undefined
    const svcDesde = fchServDesde ? formatDateArca(fchServDesde) : undefined;
    const svcHasta = fchServHasta ? formatDateArca(fchServHasta) : undefined;
    const svcPago = fchVtoPago ? formatDateArca(fchVtoPago) : undefined;

    // Step 3: Request CAE from ARCA WSFE
    const result = await requestCAE(
      cuit,
      {
        ...
        DocNro: parseInt(clienteDoc),
        CondicionIVAReceptorId: condicionIva, 
        CbteDesde: nextVoucher,
        CbteHasta: nextVoucher,
        CbteFch: fechaFactura,
        FchServDesde: svcDesde,
        FchServHasta: svcHasta,
        FchVtoPago: svcPago,
        ImpTotal: importeNum,
        ImpTotConc: 0,
        ImpNeto: importeNum,
    ...
    const vencimientoCae: string = result.CAEFchVto;
